...
def test_img2graph(shape, threshold):
    ...
    if N < 256 * 256:
        ...
        xy = xy * 10
        plt.subplot(121)
        draw_cn(Graph(Ar), pos=xy, node_color=beta_r, font_size=5, node_size=10, with_labels=False)
        plt.subplot(122)
        draw_cn(Graph(Ad), pos=xy, node_color=beta_d, font_size=5, node_size=10, with_labels=False)
        plt.show()
