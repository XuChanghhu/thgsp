...
class TestQmfCore:
    def test_init(self, device, dtype):
        ...
        QmfCore(Bs, torch.as_tensor(beta_np), analyze_kernels=(meyer_kernel, meyer_mirror_kernel))

        qmf = QmfCore(Bs, beta_np, in_channels=3)
        assert qmf.coefficient_a.shape == (M, 2 ** M, 3, qmf.order + 1)  # 20 is the default order of approximation
    ...
    def test_transform(self, device, dtype):
        ...
        bpg, bt = rand_bipartite(N // 2, N - N // 2, p=0.2, dtype=dtype, device=device, return_partition=True)
        Bs.append(bpg)
        beta.append(bt)
        ...
        print("max distortion among all nodes:  ", dis.max(), " at {}-th node".format(dis.argmax()))
        print("min distortion among all nodes:  ", dis.min(), " at {}-th node".format(dis.argmin()))
...
class TestColorQmf:
    ...
    def test_transform(self, dtype, device, strategy, Ci):
        ...
        print("\n----- Strategy: {:8s}, M={:4d}  Device: {:5s}, Dtype: {:6s} -----------".format(strategy, M,
                                                                                                 str(device),
                                                                                                 str(dtype)))
        print("|reconstruction SNR:    ", snr(f_hat, f))
        print("|max distortion among all nodes:  ", dis.max().item(), " at {}-th node".format(dis.argmax()))
        print("|min distortion among all nodes:  ", dis.min().item(), " at {}-th node".format(dis.argmin()))
...
class TestNumQMf:
    ...
    def test_transform(self, dtype, device, strategy, M):
        ...
        print("\n|------------ Strategy: {:8s}, M={}, Device: {:5s}, Dtype: {:6s} -----------".format(strategy, M,
                                                                                                      str(device),
                                                                                                      str(dtype)))
        print("reconstruction SNR:  ", snr(f_hat, f).item())
        print("max distortion among all nodes:  ", dis.max(), " at {}-th node".format(dis.argmax()))
        print("min distortion among all nodes:  ", dis.min(), " at {}-th node".format(dis.argmin()))
        print("distortion sum                :  ", dis.sum())
        print("|---------------------------------------------------------------------------------")
...
class TestBiorthCore:
    def test_init(self, dtype, device):
        ...
        assert bio.coefficient_a.shape == (M, 2 ** M, 3, 16 + 1)  # 16 is the default order of approximation
...
class TestColorBiorth:
    ...
    def test_transform(self, dtype, device, strategy):
        ...
        print("\n|----- Strategy: {:8s}, M:{:4d}  Device: {:5s}, Dtype: {:6s} -----------".format(strategy, M,
                                                                                                  str(device),
                                                                                                  str(dtype)))
        print("reconstruction SNR:    ", snr(f_hat, f).item(), " dB")
        print("max distortion among all nodes:  ", dis.max(), " at {}-th node".format(dis.argmax()))
        print("min distortion among all nodes:  ", dis.min(), " at {}-th node".format(dis.argmin()))
        print("distortion sum                :  ", dis.sum())
        print("|---------------------------------------------------------------------------------")
...
class TestQmfWaveletBasis:
    def test_one_level(self, dtype, device):
        ...
        basis = QmfOperator([bptG.to_scipy("csr")], beta.view(-1, 1), order=20, device=device)
...
class TestBiorWaveletBasis:
    def test_one_level(self, dtype, device):
        ...
        basis = BiorthOperator([bptG.to_scipy("csr")], beta.view(-1, 1), k=2, device=device)
