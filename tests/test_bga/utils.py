...
from ..utils4t import float_dtypes, lap_types
from thgsp.bga.utils import *
...
class TestIbFix:
    def test_bipartite(self):
        ...
        for i in range(n_sample):
            ...
            flag, vtx_color, _ = is_bipartite_fix(B.adj.to_scipy("csr"), fix_flag=False)
            assert flag
            NB = rand_udg(N, 0.8)  # complete graph must be non-bipartite
            flag, vtx_color, _ = is_bipartite_fix(NB.adj.to_scipy("csr"), fix_flag=False)
            assert not flag

    def test_bipartite_fix(self):
        N = 12
        B = rand_udg(N, 0.3)
        flag, vtx_color, Bb = is_bipartite_fix(B.adj.to_scipy('csr'), fix_flag=True)
        ...
        draw_cn(nx.from_scipy_sparse_matrix(Bb), pos, vtx_color, ax=axes[1])
        plt.show()
    ...
    def test_bipartite_fix_th(self):
        ...
        N = N1 + N2
        B = rand_udg(N, 0.6)
        flag, vtx_color, Bb = is_bipartite_fix(B.adj.to_dense(), fix_flag=False)
        assert not flag  # if fix_flag, always bipartite

        B = rand_bipartite(N1, N2, 0.6)
        flag, vtx_color, _ = is_bipartite_fix(B.adj.to_dense(), fix_flag=False)
        assert flag
