...
class TestUtils:
    def test_dkl(self, p, dt):
        ...
        N2 = 10
        delta = 0.1
        Blil = rand_bipartite(N1, N2, p, dt).adj.to_scipy('csr').tolil()
    ...
    def test_compute_sigma(self, p, dt):
        N = 100
        delta = 0.1
        Acoo = rand_udg(N, p, dt).adj.to_scipy('coo')
...
class TestAmfs:
    def test_amfs1level(self, dt):
        N = 100
        delta = 0.1
        W = rand_udg(N, dtype=dt).adj.to_scipy('csr').tolil()
