...
def random_graph(N, density=0.01, directed=False, dtype=None, device=None, weighted=True, seed=None):
    ...
    if not directed:
        ...
        if k<1:
            raise RuntimeError(f"Density {density} is too small to generate 1 edge of {N}-node undirected graph.")
        ...
        k = int(density * n_all_edge)
        if k < 1:
            raise RuntimeError(f"Density {density} is too small to generate 1 edge of {N}-node directed graph.")
        ind = random_state.choice(n_all_edge, size=k, replace=False)
        data = random_state.rand(k) if weighted else None
    ...
    coo_adj = SparseTensor(row=row, col=col, value=value, sparse_sizes=(N, N)).to(device)
...
def knn(x, k=2, loop=False, dtype=None, device=None):
    ...
    return SparseTensor(row=edge_index[0], col=edge_index[1], value=edge_val)
