...
def get_kernel_name(kernel_array, identation=False):
    get_name = np.vectorize(lambda f: '_'.join(f.__name__.split('_')[:-1]) if '_' in f.__name__ else f.__name__)
...
def design_p(K):
    ...
    for i in range(K):
        A[i:i + K + 1, i] = c1
    A[:, 1::2] = -A[:, 1::2]  # due to the expansion of (1-k)^K which includes (-1)^K for all odd degree terms
    A_even = A[::2]
    c_p_even = np.zeros(K)
    c_p_even[0] = 1.
...
def get_kernel_name(kernel_array, indentation=False):
    get_name = np.vectorize(lambda f: '_'.join(f.__name__.split('_')[:-1]) if '_' in f.__name__ else f.__name__)
    info = get_name(kernel_array)
    return "\t" + str(info).replace('\n', '\n\t') if indentation else str(info)
