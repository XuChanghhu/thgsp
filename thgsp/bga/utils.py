...
def beta2color_group(beta, th=True):
    ...
    colors = np.packbits(beta_dist, axis=1, bitorder='little').reshape(-1).tolist()  # n_channel x 1 --> n_channel

    node_colors = np.packbits(beta, axis=1, bitorder='little').reshape(-1)  # array(N x 1) --> List(N)
    color_group = {color: None for color in colors}  # some channels may be empty
...
def beta2channel_mask(beta):
    ...
    beta_dist = distribute_color(n_channel, M, th=False)  # n_channel x M np.array
    colors = np.packbits(beta_dist, axis=1, bitorder='little').reshape(-1)  # n_channel x 1 --> n_channel
...
def is_bipartite_fix_scipy(A, fix_flag: bool = False):
    ...
    vtx_color = [-1 for _ in range(n_node)]  # the same initial color for all nodes
    flag = True
